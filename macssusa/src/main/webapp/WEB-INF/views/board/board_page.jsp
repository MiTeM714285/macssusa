...
<div>
	<c:if test="${page.prev}">
 		<span>[ <a href="/board/board_page?num=${page.startPageNum - 1}&btype=${param.btype}">이전</a>]</span>
	</c:if>
		...
		<span>
			<c:choose>
				<c:when test="${selected != num}">
					<a href="/board/board_page?num=${num}&btype=${param.btype}">${num}</a>
				</c:when>
				<c:otherwise>
					<b>${num}</b>
				</c:otherwise>
	...
	<c:if test="${page.next}">
	 	<span>[ <a href="/board/board_page?num=${page.endPageNum + 1}&btype=${param.btype}">다음</a>]</span>
	</c:if>
</div>
